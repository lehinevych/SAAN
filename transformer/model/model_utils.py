...
def get_embedding_padding(x, padding_value=0, dtype=tf.float32):

  with tf.name_scope('embedded_padding'):
    means, variances = tf.nn.moments(x, axes=-1)
    mask = tf.logical_and(tf.equal(means, padding_value), tf.equal(variances, padding_value))
...
def get_padding_bias(x):
  ...
  with tf.name_scope("attention_bias"):
    padding = get_embedding_padding(x)
